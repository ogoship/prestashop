  ...
  public function export_all_products(){
	...
	foreach($products as $key=>$value){
		if($value['export_to_ogoship']==0){
            $image = Image::getCover((int)$value['id_product']);
			$imagePath = $this->context->link->getImageLink($value['link_rewrite'], $image['id_image'], 'home_default');
  ...
  public function updateProductStockStatus($strSku,	$strQuantity){
    ...
    public function hookDisplayBackOfficeOrderActions($params)
    {
        $this->context->smarty->assign(
            [
                'ogoship' => Configuration::get('ogoship'),
                'id_order' => $params['id_order'],
            ]
        );

        return $this->display(__FILE__, 'sendtoogoship.tpl');
    }
    ...
    public function hookDisplayAdminProductsShippingStepBottom($params)
    {
        ...
        return $this->display(__FILE__, "denyexport.tpl");
    }
}